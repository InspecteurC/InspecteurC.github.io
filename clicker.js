...
var display = {
	...
	updateShop: function() {
		...
		for (i=0;i<Buildings.name.length;i++) {
			document.getElementById("buildsContainer").innerHTML+='<div class="ShopButton '+Buildings.prodType[i]+' unselectable"><table><td id="name"><p class="name">'+Buildings.name[i]+' : </p></td><td><span class="count">'+Buildings.Number[i]+'</span></td></tr><tr><td><ul><li class="'+Buildings.prodType[i]+'"><button onclick="Buildings.buy('+i+',1)">+1</button>Cost: <span>'+expOrFix(Buildings.buildingCost(i,1))+'</span></li><li class="'+Buildings.prodType[i]+'"><button onclick="Buildings.buy('+i+',10)">+10</button>Cost: <span>'+expOrFix(Buildings.buildingCost(i,10))+'</span></li><li class="'+Buildings.prodType[i]+'"><button onclick="Buildings.buy('+i+',50)">+50</button>Cost: <span>'+expOrFix(Buildings.buildingCost(i,50))+'</span></li></td></tr></table><div>';
		}
	},
	updateCountries: function() {
		...
		for (i=0;i<Enemies.name.length;i++)/*check the entire Array "Enemies"*/ {
			if (Enemies.setMilitaryPower(i) <= game.Weapons && Enemies.status[i] != "Defeated" ) {
				...
				document.getElementById("EnemiesList").innerHTML+='<li class="upgradeButton '+Enemies.status[i]+' unselectable"> <p>'+Enemies.name[i]+'<span> '+expOrFix(Enemies.setMilitaryPower(i))+'</span> Weapons to conquer.</p></li>';
			}
		}
	},
	updateUpgrades : function() {
		...
		for(i=0;i<Upgrades.name.length;i++) {
			...
			document.getElementById("upgradesContainer").innerHTML+='<div class="'+Upgrades.type[i]+' '+Buildings.prodType[Upgrades.effectOn[i]]+' unselectable"><table><tr><td class="name"><p>'+Upgrades.name[i]+': </p></td><td class="count"> '+Upgrades.number[i]+'</td></tr><tr><td class="'+Buildings.prodType[Upgrades.effectOn[i]]+'"><button onclick="Upgrades.buy('+i+')">Buy this '+Upgrades.type[i]+'</button></td><td> Cost:'+Upgrades.costCalc[i](Upgrades.number[i])+'</td><td>'+Upgrades.costType[i]+'</td></tr><tr><td> '+Upgrades.description[i]+'</td></tr></table></div>';
			}
		}
	}
};
...
var Upgrades = {
	name:["Better Pickaxes","Higher QUality Ores"],
	...
	status:[],
	description:["Increase Gold Per Secound","Increase Weapons production"],
...
var Enemies = {
	...
	setMilitaryPower: function(index) {
		this.militaryPower[index] = Math.pow(50+20*index+Math.pow(index,2),2);
...
function saveGame() {
	var gameSave = {
		...
		buildingProd: Buildings.baseProd,
...
function loadGame() {
	...
	if (localStorage.getItem("gameSave")!== null) {
		...
		if (typeof savedGame.buildingProd !== "undefined") {
			for (i=0;i<savedGame.buildingProd.length; i++) {
				Buildings.baseProd[i] = savedGame.buildingProd[i];
			}
		}
